...
namespace AI.StateMachine
{
	public class StateMachine
	{
			...
			params TransitionCondition[] conditions)
		{
			if (fromState == null)
			{
				throw new ArgumentNullException(
					nameof(fromState),
					"Cannot add a transition that has an empty state");
			}
		...
		public bool GetBool(string key) => stateData.GetBoolean(key);
		...
		/// <param name="key"></param>
		/// <returns></returns>
		public bool GetTrigger(string key) => stateData.GetTrigger(key);
		...
		public void Start()
		{
			if (entryState == null)
			{
				throw new InvalidOperationException(
					"State machine must have an entry state defined to start.");
			}
